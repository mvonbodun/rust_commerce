...
pub fn load_environment() {
    ...
    let env_specific_file = match environment.as_str() {
        ...
        "staging" => ".env.staging",
        "local" => ".env.local",
        _ => ".env.local", // Default to local for any other value
    };
    ...
    match dotenvy::from_path(".env") {
        ...
        Err(_) => {
            ...
            if dotenvy::dotenv().is_ok() {
                loaded_files.push("default .env".to_string());
            }
        }
    }
    ...
    if loaded_files.is_empty() {
        ...
        println!(
            "🔧 Loaded environment layers: {} (→ higher priority overrides lower)",
            loaded_files.join(" → ")
        );
        println!(
            "🔧 Final precedence: system env > {} > base defaults",
            loaded_files.first().unwrap_or(&"none".to_string())
        );
    }
}
