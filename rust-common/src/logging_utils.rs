...
pub async fn validate_dependencies(
    ...
    let databases = mongo_client.list_database_names().await
        .with_context("Failed to list MongoDB databases")?;
    debug!("✅ MongoDB connection validated, {} databases available", databases.len());
    
    // Test NATS subscription capability
    match nats_client.queue_subscribe("health_check", "test_queue".to_string()).await {
        Ok(_) => {
