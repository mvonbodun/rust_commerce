...
mod tests {
    ...
    fn test_validation_logic() {
        ...
        let skus: Vec<String> = (0..101).map(|i| format!("SKU-{:03}", i)).collect();
        ...
        assert!(!skus.is_empty() && skus.len() <= 100, "Valid SKUs list should pass");
        ...
        let currency = "USD";
        assert!(currency == "USD" || currency == "EUR", "Valid currency should pass");
        
        let currency = "GBP";
        assert!(!(currency == "USD" || currency == "EUR"), "Invalid currency should fail");
    ...
    async fn test_handler_error_types() {
        ...
        impl OfferDao for MockErrorDao {
            async fn create_offer(&self, _offer: Offer) -> Result<Offer, rust_price::model::DBError> {
                unimplemented!()
            }
            
            async fn delete_offer(&self, _offer_id: String) -> Result<(), rust_price::model::DBError> {
                unimplemented!()
            }
            
            async fn get_offer(&self, _offer_id: String) -> Result<Option<Offer>, rust_price::model::DBError> {
                unimplemented!()
            }
            ...
            async fn find_best_offer_prices(
                ...
                Err(rust_price::model::DBError::Other(Box::new(
                    std::io::Error::new(std::io::ErrorKind::Other, "Mock error")
                )))
            }
        }
        ...
        match result {
            Err(HandlerError::ValidationError(msg)) => {
                assert!(msg.contains("empty"), "Should be a validation error about empty SKUs");
            }
            _ => panic!("Expected ValidationError for empty SKUs list"),
        }
        
        // Test too many SKUs
        let many_skus: Vec<String> = (0..101).map(|i| format!("SKU-{:03}", i)).collect();
        let result = get_best_offer_prices(
            many_skus,
            5,
            None,
        ...
        match result {
            Err(HandlerError::ValidationError(msg)) => {
                assert!(msg.contains("Too many SKUs"), "Should be a validation error about too many SKUs");
            }
        ...
        match result {
            Err(HandlerError::InternalError(msg)) => {
                assert!(msg.contains("Failed to find"), "Should be an internal error from DAO");
            }
    ...
    fn test_sku_parsing_and_deduplication() {
        ...
        let mut skus = vec!["SKU-001".to_string(), "SKU-002".to_string(), "SKU-001".to_string()];
        skus.sort();
        skus.dedup();
    ...
    fn test_response_aggregation_logic() {
        ...
        let sku_results: Vec<SkuOfferResult> = results.into_iter().map(|(sku, offer_option)| {
