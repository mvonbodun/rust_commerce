...
pub async fn get_best_offer_prices(
    ...
    debug!("Before call to get_best_offer_prices for {} SKUs", skus.len());

    // Validate input parameters
    if skus.is_empty() {
        return Err(HandlerError::ValidationError("SKUs list cannot be empty".to_string()));
    }

    if skus.len() > 100 {
        return Err(HandlerError::ValidationError(
            format!("Too many SKUs provided. Maximum is 100, got {}", skus.len())
        ));
    }
    ...
    for sku in &skus {
        if sku.trim().is_empty() {
            return Err(HandlerError::ValidationError("All SKUs must be non-empty".to_string()));
        }
    }
    ...
    debug!(
        "Validated parameters - {} SKUs, quantity: {}, date: {}, currency: {}",
        skus.len(), quantity, parsed_date, currency
    );
    ...
    match result {
        ...
        Err(e) => {
            ...
            Err(HandlerError::InternalError(format!(
                "Failed to find best offer prices: {}",
                e
            )))
        }
    }
