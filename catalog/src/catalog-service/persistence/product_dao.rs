...
pub trait ProductDao {
    ...
    async fn get_product_slugs_paginated(&self, batch_size: i32, cursor: Option<String>, include_inactive: bool) -> Result<(Vec<String>, Option<String>, bool), Box<dyn Error + Send + Sync>>;
}
...
impl ProductDao for ProductDaoImpl {
    ...
    async fn get_product_slugs_paginated(
        ...
        let batch_size = if batch_size < 10 { 10 } else if batch_size > 1000 { 1000 } else { batch_size };
        ...
        let slugs: Vec<String> = product_slugs.iter().take(items_to_return).map(|ps| ps.slug.clone()).collect();
