...
use crate::services::product_service::HandlerError;
    ...
    match request {
        Ok(request) => {
            let result = app_state
                .product_service
                .create_product(
                    request, None, // created_by - could be extracted from metadata
                )
                .await;
            ...
            match result {
                ...
                Err(HandlerError::ValidationError(error_msg)) => {
                ...
                Err(HandlerError::AlreadyExists(error_msg)) => {
                    warn!("Duplicate product: {error_msg}");
                    let response = ProductCreateResponse {
                        product: None,
                        status: Some(catalog_messages::Status {
                            code: catalog_messages::Code::AlreadyExists.into(),
                            message: error_msg,
                            details: vec![],
                        }),
                    };
                ...
                Err(HandlerError::NotFound(error_msg)) => {
                    warn!("Not found error creating product: {error_msg}");
                    let response = ProductCreateResponse {
                        product: None,
                        status: Some(catalog_messages::Status {
...
pub async fn get_product(
    ...
    match request {
        Ok(request) => {
            ...
            let result = app_state.product_service.get_product(request.id).await;
            ...
            match result {
                ...
                Err(HandlerError::AlreadyExists(error_msg)) => {
                    // This shouldn't happen for get operations, but handle it anyway
                    warn!("Unexpected AlreadyExists error getting product: {error_msg}");
                    let response = ProductGetResponse {
                        product: None,
                        status: Some(catalog_messages::Status {
                            code: catalog_messages::Code::AlreadyExists.into(),
...
pub async fn get_product_by_slug(
    ...
    match request {
        Ok(request) => {
            let result = app_state
                .product_service
                .get_product_by_slug(request.slug)
                .await;
...
pub async fn update_product(
    ...
    match request {
        Ok(request) => {
            let result = app_state
                .product_service
                .update_product(request.id.clone(), request)
                .await;
...
pub async fn delete_product(
    ...
    match request {
        Ok(request) => {
            let result = app_state.product_service.delete_product(request.id).await;
...
pub async fn get_product_slugs(
    ...
    match request {
        Ok(request) => {
            let result = app_state
                .product_service
                .get_product_slugs(request.batch_size, request.cursor, request.include_inactive)
                .await;
