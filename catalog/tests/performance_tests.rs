...
mod performance_tests {
    ...
    fn test_large_category_hierarchy_creation() {
        ...
        for i in 0..10 {
            ...
            let root_category = Category {
                ...
                short_description: format!("Root category {i} description"),
            ...
            for j in 0..100 {
                ...
                let child_category = Category {
                    ...
                    short_description: format!("Child category {i}-{j} description"),
                    full_description: None,
                    path: format!("root-category-{i}.child-category-{i}-{j}"),
                ...
                for k in 0..10 {
                    ...
                    let grandchild_category = Category {
                        ...
                        short_description: format!(
                            "Grandchild category {i}-{j}-{k} description",
                        ),
                        full_description: None,
                        path: format!(
                            "root-category-{i}.child-category-{i}-{j}.grandchild-category-{i}-{j}-{k}"
                        ),
    ...
    println!("Found category by path in {search_duration:?}");
        ...
        assert!(
            ...
            "Category creation took too long: {creation_duration:?}",
        );
        assert!(
            traversal_duration.as_millis() < 100,
            "Hierarchy traversal took too long: {traversal_duration:?}",
        );
        assert!(
            search_duration.as_micros() < 1000,
            "Path search took too long: {search_duration:?}",
        );
    }
    ...
    println!("Generated {} paths in {duration:?}", paths.len());
        ...
        assert!(
            ...
            "Path generation took too long: {duration:?}",
        );
    ...
    println!("Built {} ancestor arrays in {duration:?}", ancestors_arrays.len());
        ...
        assert!(
            duration.as_millis() < 100,
            "Ancestor array building took too long: {duration:?}",
        );
    }
}
