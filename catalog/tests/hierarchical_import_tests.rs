...
fn test_hierarchical_slug_validation() {
    ...
    for slug in valid_slugs {
        ...
        assert!(
            !slug.contains("//"),
            "Valid slug '{slug}' should not contain double slashes"
        );
    }
    ...
    for slug in invalid_slugs {
        if !slug.is_empty() {
            assert!(
                ...
                "Invalid slug '{slug}' should fail validation"
            );
        }
    }
...
fn test_parent_child_relationship_validation() {
    ...
    for (parent_slug, child_slug, should_be_valid) in test_cases {
        let is_valid_child =
            child_slug.starts_with(&format!("{parent_slug}/")) && child_slug != parent_slug;

        assert_eq!(
            is_valid_child, should_be_valid,
            "Child slug '{child_slug}' validation against parent '{parent_slug}' should be {should_be_valid}"
        );
    }
...
fn test_dependency_sorting_logic() {
    ...
    assert!(
        sorted[1].slug == "/electronics/smartphones" || sorted[1].slug == "/electronics/computers"
    ); // Children next
...
fn test_circular_dependency_detection() {
    ...
    let circular_categories = vec![
        TestCategory {
            slug: "/a".to_string(),
            parent_slug: Some("/b".to_string()),
        },
        TestCategory {
            slug: "/b".to_string(),
            parent_slug: Some("/a".to_string()),
        },
    ];
    ...
    while !remaining.is_empty() && iteration < max_iterations {
        ...
        if !progress_made {
            ...
            assert!(
                ...
                "Should have remaining categories when circular dependency detected"
            );
...
fn test_field_completeness_validation() {
    ...
    assert!(
        sample_data["short_description"].is_string(),
        "short_description should be string"
    );

    // Validate new fields
    assert!(
        sample_data["is_active"].is_boolean(),
        "is_active should be boolean"
    );
    assert!(
        sample_data["full_description"].is_string(),
        "full_description should be string"
    );
    ...
    assert!(
        seo["meta_description"].is_string(),
        "meta_description should be string"
    );
...
fn test_sample_import_data_structure() {
    ...
    for category in &sample_categories {
        ...
        if let Some(parent) = parent_slug {
            assert!(
                slug.starts_with(&format!("{parent}/")),
                "Child slug '{slug}' should be under parent '{parent}'",
            );
        }
    }
