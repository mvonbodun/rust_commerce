...
# Inventory Docker Container Management Script
# Usage: ./inventory-docker.sh <env> <action>
# env: local|production (matches .env.local or .env.production)
# action: start|stop

CONTAINER_NAME="rust-commerce-inventory-service"
IMAGE_NAME="inventory-service:latest"
...
show_help() {
    echo "Inventory Service Docker Container Manager"
    echo ""
