...
mod inventory_multi_sku_integration_tests {
    ...
    use inventory_messages::{
        InventoryCreateRequest, InventoryCreateResponse,
        InventoryGetAllLocationsBySkuRequest, InventoryGetAllLocationsBySkuResponse,
        InventoryDeleteRequest, InventoryDeleteResponse,
        Code
    };
    ...
    async fn create_test_inventory_item(
        ...
        if let Some(status) = &create_response.status {
            if status.code == Code::Ok as i32 {
                ...
                Err(format!("Failed to create inventory item: {} ({})", status.message, status.code).into())
            }
        } else {
    ...
    async fn cleanup_inventory_item(client: &async_nats::Client, sku: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn test_get_all_locations_by_sku_success() {
        ...
        let sku1 = format!("TEST-SKU-{}", Uuid::new_v4().to_string()[0..8].to_uppercase());
        let sku2 = format!("TEST-SKU-{}", Uuid::new_v4().to_string()[0..8].to_uppercase());
        
        // SKU1 in multiple locations
        create_test_inventory_item(&client, &sku1, "DC - Dallas, TX", 100, 20).await
            .expect("Failed to create test inventory item");
        create_test_inventory_item(&client, &sku1, "STORE - Spring, TX", 50, 10).await
            .expect("Failed to create test inventory item");
        create_test_inventory_item(&client, &sku1, "STORE - Austin, TX", 30, 5).await
            .expect("Failed to create test inventory item");
        
        // SKU2 in single location
        create_test_inventory_item(&client, &sku2, "DC - Dallas, TX", 75, 15).await
            .expect("Failed to create test inventory item");
        ...
        let sku1_summary = get_response.sku_summaries.iter()
            .find(|s| s.sku == sku1)
        ...
        let sku2_summary = get_response.sku_summaries.iter()
            .find(|s| s.sku == sku2)
        ...
        // Cleanup
        cleanup_inventory_item(&client, &sku1).await.expect("Failed to cleanup SKU1");
        cleanup_inventory_item(&client, &sku2).await.expect("Failed to cleanup SKU2");
    }
    ...
    async fn test_get_all_locations_by_sku_partial_results() {
        ...
        let existing_sku = format!("TEST-EXISTING-{}", Uuid::new_v4().to_string()[0..8].to_uppercase());
        let non_existing_sku = format!("TEST-MISSING-{}", Uuid::new_v4().to_string()[0..8].to_uppercase());
        
        create_test_inventory_item(&client, &existing_sku, "DC - Dallas, TX", 50, 10).await
            .expect("Failed to create test inventory item");
        ...
        // Cleanup
        cleanup_inventory_item(&client, &existing_sku).await.expect("Failed to cleanup existing SKU");
    }
    ...
    async fn test_get_all_locations_by_sku_performance() {
        ...
        for i in 0..10 {
            ...
            create_test_inventory_item(&client, &sku, "DC - Dallas, TX", 100, 20).await
                .expect("Failed to create test inventory item");
            test_skus.push(sku);
        }
        ...
        assert!(duration.as_millis() < 500, "Response time {}ms exceeds 500ms threshold", duration.as_millis());

        // Cleanup
        for sku in &test_skus {
            cleanup_inventory_item(&client, sku).await.expect("Failed to cleanup test SKU");
        }
    }
    ...
    async fn test_get_all_locations_by_sku_aggregation_accuracy() {
        ...
        let test_sku = format!("AGGREGATION-TEST-{}", Uuid::new_v4().to_string()[0..8].to_uppercase());
        
        // Create items with specific values for precise aggregation testing
        create_test_inventory_item(&client, &test_sku, "Location A", 100, 25).await
            .expect("Failed to create test inventory item");
        create_test_inventory_item(&client, &test_sku, "Location B", 200, 50).await
            .expect("Failed to create test inventory item");
        create_test_inventory_item(&client, &test_sku, "Location C", 150, 30).await
            .expect("Failed to create test inventory item");
        ...
        let locations: HashMap<String, &inventory_messages::InventoryLocationDetail> = summary.location_details
            .iter()
        ...
        // Cleanup
        cleanup_inventory_item(&client, &test_sku).await.expect("Failed to cleanup test SKU");
    }
}
