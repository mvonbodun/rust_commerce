    ...
    match result {
        ...
        Err(e) => {
            ...
            Err(HandlerError::InternalError(format!(
                "Failed to create inventory item: {e}"
            )))
        }
    }
}
...
pub async fn get_item(
    ...
    match result {
        ...
        Err(e) => {
            ...
            Err(HandlerError::InternalError(format!(
                "Failed to get inventory item: {e}"
            )))
        }
    }
}
...
pub async fn update_stock(
    ...
    let result = inventory_dao
        .update_stock(sku, quantity_change, reason)
        .await;
...
pub async fn get_all_locations_by_sku(
    ...
    match result {
        Ok(inventory_by_sku) => {
            ...
            let found_skus: std::collections::HashSet<String> =
                inventory_by_sku.keys().cloned().collect();
            let requested_skus: std::collections::HashSet<String> = skus.into_iter().collect();
            let not_found_skus: Vec<String> =
                requested_skus.difference(&found_skus).cloned().collect();

            debug!(
                "Found {} SKUs, {} not found",
                found_skus.len(),
                not_found_skus.len()
            );
        ...
        Err(e) => {
            ...
            Err(HandlerError::InternalError(format!(
                "Failed to get inventory items by SKUs: {e}"
            )))
        }
    }
}
